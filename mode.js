...
document.getElementById('dijkstraButton').addEventListener('click', function() {
    ...
    for (const edge of edgeList) {
        ...
        arc.next = nodeList[start].first;
        nodeList[start].first = arc;
    }

    // Reset the nodes
    for (let i = 0; i < nodeCount; i++) {
        nodeList[i].distance = 99999999; // LARGE
        nodeList[i].P = -1;
    ...
    //console.log(nodes);
    DijkstraHeap(nodeList, Origin, nodeCount);

    console.log("PRINTING RESULTS:");
    printOutput(nodeList, Origin, nodeCount);
